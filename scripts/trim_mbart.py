# from https://github.com/pytorch/fairseq/issues/2120#issuecomment-647429120
...
def main() -> None:
    ...
    pre_dict = load_dict(langs, os.path.join(args.pre_train_dir, "dict_orig.txt"))
    ft_dict = load_dict(langs, args.ft_dict)
    data = torch.load(os.path.join(args.pre_train_dir, "model_orig.pt"), map_location=torch.device('cpu'))
    model = data["model"]
